    ...
    <Link to={`/events/${event.id}`}>
        <div className="event__card" data-aos="fade-up">
            <div className="event__card-img">
                <img src={event.image && event.image[0]} alt={event.title} />
            </div>
...
const Events = () => {
  ...
  return (
    <section className="app__events section__padding" id="events">
        <div className="app__events-header" data-aos="fade-down">
            <h1 className="heading-text">События и Дегустации</h1>
        ...
        <div style={{ textAlign: 'center', marginTop: '3rem' }}>
            <Link to="/events">
                <button type="button" className="custom__button" data-aos="fade-up">Все События</button>
            </Link>
        </div>
    </section>
  );
